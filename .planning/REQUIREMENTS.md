...
- [x] **TERM-UX01**: Diagnosis + plan narrative render with terminal-styled headings, code blocks, inline code, bold/italic, lists (not raw `##` / triple-backtick / `**` chars). Single `MarkdownView` component shared across DPEVPanel + replay.
- [ ] **TERM-UX02**: Every executed step shows the actual command string, exit code, and stdout preview (first N lines, configurable). Stderr shown when exit ≠ 0. No more `[1/0] ✓ []` with empty brackets. <!-- 19.3-06: un-checked to match its Planned row. StepCard renders command/risk/target/stdout/stderr but NO exit code, and replay carries no output at all (executor.ts:341 logs only {stepIndex, command, exitCode}). -->
...
- [ ] **TERM-UX04**: Keyboard navigation (↑/↓ + j/k + Enter / Esc) moves a selection cursor through phase headers. Enter on a completed phase expands its full stream; Esc collapses. The DIAGNOSIS Markdown text stays accessible after PLAN begins. <!-- 19.3-06: now genuinely REACHABLE — App.tsx arbitrates a single keyboard owner, so j/k/n no longer leak into the prompt and Esc collapses before exiting. Stays Planned only because no test can drive a live SSE session, so "cursor actually moves" is unobserved. First item for the phase-closing UAT. -->
- [ ] **TERM-UX05**: `s` / Esc dashboard shows non-zero values after ≥1 session: Cache hit-rate, total entries, memory incident count, context-window % of the live session, latest call usage. Polls every 2-3s while overlay open. <!-- 19.3-06: un-checked to match its Planned row. Poll + cache split + live context-window % are in (19.3-05, 19.3-06 item B), but /status never emits latestCall, so "latest call usage" still reads "No LLM calls yet". -->
...
- [ ] **TERM-UX08**: Selecting an older session from the Sessions list shows the complete D→P→E→V stream including every step's command + output, not just the phase headers. Initial state = all phases collapsed. <!-- 19.3-06: UI side complete — buildReplayState reconstructs command/risk/target/total/stdout/stderr by stepIndex merge and returns expandedPhases:{}. Blocked on the producer: executor.ts:341 logs only {stepIndex, command, exitCode} and step_failed is never emitted, so step OUTPUT cannot replay. See deferred-items.md. -->
...
| TERM-UX01 | Phase 19.3 | Complete |
| TERM-UX02 | Phase 19.3 | Planned |
| TERM-UX03 | Phase 19.3 | Complete |
| TERM-UX04 | Phase 19.3 | Planned |
| TERM-UX05 | Phase 19.3 | Planned |
| TERM-UX06 | Phase 19.3 | Complete |
